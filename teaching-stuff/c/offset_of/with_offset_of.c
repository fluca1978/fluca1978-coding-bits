 ...
 * the compiler offsetof instruction (actually wrapped by the c library).
 *
 * Compile the program with:
 * gcc -lm -O0 with_offset_of.c
 * and run.
 */
...
int main( int argc, char** argv ){
  ...
  */
  ...
  // cast the pointer to the packet to a pointer to the header (the offset here is null)
  offset = offsetof( struct packet_with_header_at_top, p_header );
  printf( "\nOffset computed is %d", offset );
  header_pointer = ( (char* ) top_pointer )  + offset;
  ...
  top_pointer = ( struct packet_with_header_at_top* ) ( (char*) header_pointer - offset);
  printf( "\n Name of the packet with header at top:    %s", top_pointer->p_name );
  printf( "\n-------------------------------\n");
   ...
   * The compiler will give us the aligned offset.
   */
